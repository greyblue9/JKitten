...
class Kernel:
  ...
  _maxHistorySize = 1000  # maximum length of the _inputs and _responses lists
  _maxRecursionDepth = 10  # maximum number of recursive <srai>/<sr> tags before the response is aborted.
  ...
  def __init__(self, sessions=None):
    ...
    self._botPredicates = {}
    self.setBotPredicate("name", "Alice")
    ...
    for f in glob.glob(filename):
      ...
      with BytesIO(xmlstr) as sio:
        ...
        for key, tem in handler.categories.items():
          self._byfile[filename] = handler
          self._files[filename] = Path(filename).absolute().as_posix()
  ...
  def _processThink(self, elem, sessionID):
    ...
    """
    print(f"Inside think with {elem=}")
    for idx, e in islice(enumerate(elem), 2, len(elem)):
      print(f"  - processing think[{idx}] = {e!r}")
      self._stack.append(elem)
      self._processElement(e, sessionID)
      assert elem == self._stack.pop(-1)
    return ""
