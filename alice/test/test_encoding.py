...
class TestEncoding(unittest.TestCase):
  ...
  def setUp(self):
    self.k = Kernel()
    self.k.bootstrap(learnFiles="encoding.aiml", chdir=os.path.dirname(__file__))
  ...
  def test01_utf8(self):
    ...
    self._testTag("pattern with eñe", ["pattern #1 matched!"])

  def test02_utf8(self):
    """Test star pattern"""
    self._testTag("pattern with Á", ["pattern #2 matched: Á"])
  ...
  def test04_iso8859(self):
    ...
    self._testTag("pattern with Á", ["pattern #2 matched: Á"], encoding=enc)
