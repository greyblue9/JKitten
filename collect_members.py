...
def update_users():
  ...
  new_users.clear()
  new_users.extend(
    [users[uid] for uid in set(users) - set(prev_users)]
  )
  new_bots.clear()
  new_bots.extend(
    [bots[uid] for uid in set(bots) - set(prev_bots)]
  )
  if print_new and (new_users or new_bots):
    log.info(
      "Added + %d -> %d bots, + %d -> %d users",
      len(new_bots), len(bots),
      len(new_users), len(users)
    )
