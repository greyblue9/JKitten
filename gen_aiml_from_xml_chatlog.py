...
NAME_RE = re.compile(
  "(?:^|(?<=>| ))({})(?=$| |[?:;!,+?])".format("|".join(re.escape(n) for n in names)),
  re.IGNORECASE,
)
...
parsed = []
for n in msgnodes:
  if not all(k in n.attrib for k in ("time", "from", "to", "text")):
    continue
  try:
    text = BeautifulSoup(unquote(n.attrib.get("text")))
    text = next(next(next(next(text.children).children).children).children)
...
for msg in parsed:
  ...
  replies = sorted(
    (m for m in parsed if m.to == msg.sender and m.time >= msg.time and m.text),
    key=lambda i: i.time,
  )
  ...
  for inpt in inpt_parts:
    ...
    try:
      ...
      continue
    inpt = re.sub(r"(?<=\W)((?:[A-Z][a-z]*)(?:[^a-zA-Z0-9]))+", "", inpt, re.DOTALL)
