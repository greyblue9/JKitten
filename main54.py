import traceback
name_lookup = { "856229099952144464": "Dekriel", "856229099952144464": "David", "955035045716979763": "Alice", "889338065020129310": "Mikko", }
...
def translate_emojis(message: str) -> str:
  return EMOJI_REGEX.subn(
    " \\1 ", demoji.replace_with_desc(message.split("]")[-1].strip("\n: ,"))
  )[0]
...
if "LOCAL" in os.environ:
  ...
  async def get_response(message, uid, model="microsoft/DialoGPT-large"):
...
async def wolfram_alpha(inpt, uid):
  ...
  async with ClientSession() as session:
    async with session.get(
      ...
      "subpod[title!=\"Input\"][name!=\"Input\"] plaintext,",
      "pod[title!=\"Input\"][name!=\"Input\"] plaintext"
      ):
        response = str(ans.text)
        responses.setdefault(uid, []).append(response)
...
def categorize(text: str):
  ...
  if contains_seqs(text, ("who", "is")):
    question = True
    person = True
  if contains_seqs(text, ("who", "are")):
    question = True
    person = False
  if contains_seqs(text, ("who", "are")):
    question = True
    person = True
  if contains_seqs(text, ("how", "are")):
    question = True
  if contains_seqs(text, ("how", "is")):
    question = True
  if contains_seqs(text, ("when", "is")):
    question = True
  ...
  for attr in ("name", "age", "favorite", "master", "botmaster", "creator", "inventor", "boss", "friend", "buddy", "pal", "nemesis", "birthday", "job", "occupation"):
    if any(w.lower() == attr for w, pos in tagged):
      if attr not in attributes:
        attributes.append(attr)
      if {
        "my", "mine", "me", "your", "yours", "you"
      }.intersection({
        w.lower() == attr for w, pos in tagged
      }):
        person = True
  if (
      "what is my " in text.lower() 
      or "what is your " in text.lower()
  ):
    person = True
    question = True
    attrs = (
      text.lower().split("what is ")[-1].strip().split()[1:2]
    )
...
async def gpt_response(bot_message, uid="0"):
  log.debug("gpt_response(%r, %r)", bot_message, uid)
  model_infos = [
    ("microsoft/DialoGPT-large", 70),
    ("facebook/blenderbot-400M-distill",20),
    ("microsoft/DialoGPT-small", 10),
  ]
  response = ""
  while not response and model_infos:
    model, weight = random.choices(
      model_infos,
      weights=[w for k,w in model_infos]
    )[0]
    model_infos.remove((model, weight))
    
    last_input = inputs.setdefault(uid, [""])[-1]
    last_response = responses.setdefault(uid, [""])[-1]
    log.debug("querying model: %s", model)
    response = await get_response(
      bot_message, uid, model=model,
    )
    if not response:
      continue
    for b in BLACKLIST:
...
async def google(bot_message, uid="0"):
  ...
  if "SRAIXFAILED" in response:
    ...
    return await wolfram_alpha(bot_message, uid)
  for b in BLACKLIST:
    if b.lower() in response.lower() or response.lower() in b:
