  ...
  try:
    for _ in range(1):
      ...
      with message.channel.typing():
              m = re.compile("^(?:do you know |what is |what'?s |^ *)([0-9]+.*[0-9])[,?.]* *$", re.DOTALL|re.IGNORECASE) \
                 .search(bot_message)
              if m:
                try:
                  response = m.group(1) + " is " + str(eval(m.group(1).strip())) + "."
                  break
                except:
                  pass
              if ("how much " in bot_message 
                ...
                or "what is " in bot_message):
                  ...
                  response = (str(sorted(filter(
                    lambda i: i.text,
                    doc.select(
                      'pod[error=false] > subpod[title=\"\"] > plaintext'
                    )
                  ),
                    key=lambda i: len(i.text))[0].text)
                  )
                  if response:
                    break
              m = re.search(
              ...
              if m:
                chat = get_chat(uid)
                response = jnius.autoclass(
                  "org.alicebot.ab.Sraix"
                ).sraixPannous(
                  m.group(2) + m.group(4),
                  "*",
                  chat
                ).split(". ")[0].strip(".!?") + ".";
                chat.multisentenceRespond(
                  response
                )
              else:
              ...
              if not response or response == bot_message or any(
