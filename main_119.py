...
chat.multisentenceRespond(bot_message)
    
chat.multisentenceRespond(bot_message)
    
chat.multisentenceRespond(bot_message)
    
responses.setdefault(uid, [""])[-1]
...
async def on_message(message):
  ...
  pprint(cats)
  by_pos = { 
    pos:wd for wd,pos in cats["tagged"]
  }
  pronouns_pos = {
  ...
  if "age" in cats["attributes"] and cats["entities"]:
    if response := wolfram_alpha(bot_message):
  ...
  if has_personal and "name" in cats["attributes"]:
    last_response = responses.setdefault(uid, [""])[-1]
    chat = get_chat(uid)
    if response := chat.multisentenceRespond(bot_message):
      if any(
        b.lower() in response.lower() for b in BLACKLIST
      ):
        response = ""
  try:
    ...
    if not response and has_proper_noun and (
          "name" not in bot_message and
          "you" not in bot_message and
          " i " not in bot_message and
          " me" not in bot_message and
          "mine" not in bot_message and
          "our" not in bot_message and
          "um" not in bot_message
        ):
          ...
          if "SRAIXFAILED" in response:
            response = ""
    if not response or (
            any(b in response.lower() for b in map(str.lower, BLACKLIST))
            or 
            re.subn(r"[^a-z]+", "", response, re.IGNORECASE)[0] == 
            re.subn(r"[^a-z]+", "", bot_message, re.IGNORECASE)[0]
          or response == bot_message
          or any(x.lower() in (response or "").lower() for x in BLACKLIST)
        ):
          last_response = responses.setdefault(uid, [""])[-1]
          get_chat(uid).multisentenceRespond(last_response)
          response = get_chat(uid).multisentenceRespond(
            bot_message
          )
    if response.lower().startswith("your name is unknown") or "is unknown" in response.lower():
