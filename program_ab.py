...
def get_classpath() -> List[Path]:
  JAR_DIR = Path(getenv("JAR_DIR", (Path.cwd() / "lib").as_posix()))
  classpath = [
    JAR_DIR / "Ab.jar",
    JAR_DIR / "deps.jar",
  ]
...
def setup_jnius(classpath: List[Path]):
  ...
  if java_setup is None:
    ...
    jnius_config.add_classpath(*tuple(map(Path.as_posix, classpath)))
    ...
    for class_name in (
      ...
      log.debug("new protocol map for %r is: %r", class_name, pmap)
...
def java_classes_for_import(jarfile: Path) -> Dict[str, "jnius.JavaClass"]:
  log.debug("collecting Java classes for import from %r", jarfile)
  from jnius import autoclass

  with (ZipFile(jarfile, "r")) as jar:
    classes_by_simple_name = {
      p.stem: autoclass(".".join(p.parts[0:-1] + (p.stem,)))
