...
public class AIMLProcessor {
  ...
  */
  ...
  private static void categoryProcessor(Node n, List<Category> categories, String topic, String aimlFile, String language) {
    ...
    for (int j = 0; j < children.getLength(); j++) {
      ...
      Node m = children.item(j);
			  ...
			  String s = DomUtils.nodeToString(m);
      if (pattern.indexOf("BOT_NAME") != -1)
        pattern = pattern.replace("BOT_NAME", "ALICE");
      if (pattern.indexOf("BOTNAME") != -1)
        pattern = pattern.replace("BOTNAME", "ALICE");
      if (pattern.indexOf("__NAME__") != -1)
        pattern = pattern.replace("__NAME__", "ALICE");
      ...
      if (mName.equals("#text")) {
        ...
        System.err.println(
          "categoryProcessor: unexpected "+mName
          +" in "+s
        );
      }
    }
  ...
  public static String getNodeName(final Node n) {
    if (n != null) {
  ...
  public static boolean nodeNameIs(final Node n, final String name) {
    final String nodeName = getNodeName(n);
    return nodeName.equals(name.toLowerCase());
  }
  /**
  ...
  */
  public static List<Category> AIMLToCategories (String directory, String aimlFile) {
    ...
    for (int i = 0; i < nodelist.getLength(); i++)   {
      ...
      if (nodeNameIs(n, "category")) {
        categoryProcessor(n, categories, "*", aimlFile, language);
      }
      else if (nodeNameIs(n, "get")) {
        final NamedNodeMap attrs = n.getAttributes();
        if (attrs != null) {
          final Node attr = attrs.getNamedItem("name");
          if (attr != null) {
            String varName = attr.getTextContent();
            System.err.printf("varName from <get> %s\n ",
              varName);
          }
        }
      }
      else if (nodeNameIs(n, "topic")) {
        String topic = "";
        final NamedNodeMap attrs = n.getAttributes();
        if (attrs != null) {
          final Node attr = attrs.getNamedItem("name");
          if (attr != null) {
            topic = attr.getTextContent();
          }
        }
        if (topic == null || topic.isEmpty()) {
          topic = n.getNodeValue();
        }
        if (topic == null) topic = "*";
        topic = topic.trim();
             ...
             j++)
        {
          ...
          if (nodeNameIs(m, "category")) {
            categoryProcessor(
              m, categories, topic, aimlFile, language
            );
          }
        }
      }
  ...
  public static String evalTagContent(Node node, ParseState ps, Set<String> ignoreAttributes) {
    ...
    try {
      ...
      for (int i = 0; i < childList.getLength(); i++) {
        ...
        || !ignoreAttributes.contains(getNodeName(child))) {
      ...
      System.err.printf(
        "Something went wrong with evalTagContent: node=%s, ps=%s, ignoreAttributes=%s: %s\n",
        node, ps, ignoreAttributes, ex
      );
      ex.printStackTrace();
    }
  ...
  */
  private static String unevaluatedXML(String resultIn, Node node, ParseState ps) {
    ...
    MagicBooleans.trace(
      String.format(
        "in AIMLProcessor.unevaluatedXML() with " 
        + "resultIn=%s, returning: %s\n",
        StringEscapeUtils.escapeJava(resultIn),
  ...
  */
  private static String condition(Node node, ParseState ps) {
    ...
    HashSet<String> attributeNames = Utilities.stringSet("name", "var", "value", "attr");
    ...
    if (varName == null || varName.trim().equals("") || varName.equals("unknown")) {
      varName = predicate;
    }
    if (varName == null || varName.trim().equals("") || varName.equals("unknown")) {
      varName = String.valueOf(node.getNodeValue());
    }
    ...
    if (liList.size() == 0 && (value = getAttributeOrTagValue(node, ps, "value")) != null && !value.equals("unknown") && !value.equals("")  &&
           predicate != null  &&
           ps.chatSession.predicates.get(predicate).trim().equalsIgnoreCase(value.trim()))  {
           return evalTagContent(node, ps, attributeNames);
    }
    else if (liList.size() == 0 && (value = getAttributeOrTagValue(node, ps, "value")) != null   && !value.equals("unknown") && !value.equals("")  &&
        varName != null  &&
    ...
    // otherwise this is a <condition> with <li> items:
      ...
      if (value != null) {
        ...
        if (liPredicate != null && value != null  && !value.equals("unknown") && !value.equals("")  && (ps.chatSession.predicates.get(liPredicate).equalsIgnoreCase(value) ||
...
*/
  ...
  public static String predicate(Node node, ParseState ps) {
    ...
    if (ts.idTriple.containsKey(id))
      ...
      System.err.printf(
        "Missing predicate: %s, in node: %s\n ",
        id, DomUtils.nodeToString(node)
      );
      return "unknown";
    }
  }
