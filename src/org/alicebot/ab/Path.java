...
public class Path extends ArrayList<String> {
  ...
  private Path() {
     ...
     *
     * @param sentence entence to convert
     * @return entence in Path form
     */
  ...
  public static String pathToSentence(Path path) {
     ...
     *
     * @param array rray of strings
     * @return equence of strings as Path
     */
  private static Path arrayToPath(String[] array) {
    ...
    for (int i = array.length - 1; i >= 0; i--) {
      ...
      head.word = array[i];
      head.next = tail;
      if (tail == null) head.length = 1; else head.length = tail.length + 1;
      tail = head;
    }
    return head;
  }
     ...
     *
     * @param array rray of strings
     * @param index rray index
     * @return ath form
     */
  private static Path arrayToPath(String[] array, int index) {
    if (index >= array.length) return null; else {
      Path newPath = new Path();
      newPath.word = array[index];
      newPath.next = arrayToPath(array, index + 1);
      if (newPath.next == null) newPath.length = 1; else newPath.length = newPath.next.length + 1;
      return newPath;
    }
  }
