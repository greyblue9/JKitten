...
public class TestAB {
  ...
  public static void testChat(Bot bot, boolean doWrites, boolean traceMode) {
    ...
    while (true) {
      ...
      if (textLine.equals("q")) System.exit(0); else if (textLine.equals("wq")) {
        bot.writeQuit();
        System.exit(0);
      } else if (textLine.equals("sc")) sraixCache("c:/ab/data/sraixdata6.txt", chatSession); else if (textLine.equals("iqtest")) {
      ...
      } else if (textLine.equals("ab")) testAB(bot, sample_file); else {
  ...
  public static void runTests(Bot bot, boolean traceMode) {
    ...
    while (textLine != null) {
      ...
      if (textLine.equals("q")) System.exit(0); else if (textLine.equals("wq")) {
        bot.writeQuit();
        System.exit(0);
      } else if (textLine.equals("ab")) testAB(bot, sample_file); else if (textLine.equals(MagicStrings.null_input)) testOutput.writeLine(""); else if (textLine.startsWith("#")) testOutput.writeLine(textLine); else {
        String request = textLine;
        if (MagicBooleans.trace_mode) System.out.println("STATE=" + request + ":THAT=" + chatSession.thatHistory.get(0).get(0) + ":TOPIC=" + chatSession.predicates.get("topic"));
        String response = chatSession.multisentenceRespond(request);
        while (response.contains("&lt;")) response = response.replace("&lt;", "<");
