...
public class TripleStore {
  ...
  if (tripleStringId.keySet().contains(tripleString)) {
    ...
    if (subjectTriples.containsKey(s)) existingTriples = subjectTriples.get(s); else existingTriples = new HashSet<String>();
    existingTriples.add(id);
    subjectTriples.put(s, existingTriples);
    if (predicateTriples.containsKey(p)) existingTriples = predicateTriples.get(p); else existingTriples = new HashSet<String>();
    existingTriples.add(id);
    predicateTriples.put(p, existingTriples);
    if (objectTriples.containsKey(o)) existingTriples = objectTriples.get(o); else existingTriples = new HashSet<String>();
    existingTriples.add(id);
  ...
  if (s == null || s.startsWith("?")) {
    ...
    if (subjectTriples.containsKey(s)) subjectSet = subjectTriples.get(s); else subjectSet = emptySet();
  }
  ...
  if (p == null || p.startsWith("?")) {
    ...
    p = p.toUpperCase();
    if (predicateTriples.containsKey(p)) predicateSet = predicateTriples.get(p); else predicateSet = emptySet();
  }
  if (o == null || o.startsWith("?")) {
    objectSet = allTriples();
  } else {
    o = o.toUpperCase();
    if (objectTriples.containsKey(o)) objectSet = objectTriples.get(o); else objectSet = emptySet();
  }
  ...
  if (idTriple.containsKey(id)) return idTriple.get(id).subject; else return "Unknown subject";
  }

  public String getPredicate(String id) {
  if (idTriple.containsKey(id)) return idTriple.get(id).predicate; else return "Unknown predicate";
  }

  public String getObject(String id) {
  if (idTriple.containsKey(id)) return idTriple.get(id).object; else return "Unknown object";
  }
  ...
  try {
    ...
    if (MagicBooleans.trace_mode) for (Tuple t : result) {
  ...
  if (vars.contains(subj)) {
    ...
    if (!value.equals(MagicStrings.unbound_variable)) {
        /*System.out.println("adjusting "+subj+" "+value);*/
  ...
  if (vars.contains(pred)) {
    ...
    if (!value.equals(MagicStrings.unbound_variable)) {
        /*System.out.println("adjusting "+pred+" "+value);*/
  ...
  if (vars.contains(obj)) {
    ...
    if (!value.equals(MagicStrings.unbound_variable)) {
        /*System.out.println("adjusting "+obj+" "+value); */
    newClause.obj = value;
    }
  }
  return newClause;
