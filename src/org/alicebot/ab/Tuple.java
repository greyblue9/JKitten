...
public class Tuple extends HashMap<String, String> {
  ...
  public int hashCode() {
    ...
    for (String x : visibleVars) {
      ...
      if (get(x) != null) result = 31 * result + get(x).hashCode();
    }
    return result;
  }
  ...
  public String printVars() {
    ...
    for (String x : getVars()) {
      if (visibleVars.contains(x)) result = result + " " + x; else result = result + " [" + x + "]";
    }
    return result;
  }
  ...
  public String getValue(String var) {
    ...
    if (result == null) return MagicStrings.default_get; else return result;
  }

  public void bind(String var, String value) {
    if (get(var) != null && !get(var).equals(MagicStrings.unbound_variable)) System.out.println(var + " already bound to " + get(var)); else put(var, value);
  }
